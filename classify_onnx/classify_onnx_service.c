#include "classify_onnx_service.h"
...
static const char *TAG = "classify_onnx";
...
esp_err_t classifyOnnx_start(void)
{
    ...
    esp_err_t err = onnx_audio_model_init();
    if (err == ESP_OK) {
        err = acfg_init(&s_audio_classifier,
                        onnx_audio_model_invoke,
                        onnx_audio_model_ctx());
    }
